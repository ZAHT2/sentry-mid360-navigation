...
namespace
{
tf2::Transform toPlanarBaseTransform(const tf2::Transform & transform)
{
  tf2::Transform planar;
  planar.setOrigin(tf2::Vector3(transform.getOrigin().x(), transform.getOrigin().y(), 0.0));

  tf2::Quaternion yaw_only;
  yaw_only.setRPY(0.0, 0.0, tf2::getYaw(transform.getRotation()));
  planar.setRotation(yaw_only);
  return planar;
}
}  // namespace
  ...
  const sensor_msgs::msg::PointCloud2::ConstSharedPtr & pcd_msg)
{
  ...
  const tf2::Transform tf_odom_to_chassis_planar = toPlanarBaseTransform(tf_odom_to_chassis);
  const tf2::Transform tf_odom_to_robot_base_planar =
    toPlanarBaseTransform(tf_odom_to_robot_base);

  publishTransform(
    tf_odom_to_chassis_planar, odometry_msg->header.frame_id, base_frame_, pcd_msg->header.stamp);
  publishOdometry(
    tf_odom_to_robot_base_planar, odometry_msg->header.frame_id, robot_base_frame_,
